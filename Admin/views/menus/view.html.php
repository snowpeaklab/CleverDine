 ...
 * restaurants View
 */
class cleverdineViewmenus extends JViewUI {
	/**
	...
	function display($tpl = null) {
		...
		$filters = array();
		$filters['keysearch'] = $mainframe->getUserStateFromRequest('vrmenus.keysearch', 'keysearch', '', 'string');
		...
		$lim0 	= $mainframe->getUserStateFromRequest('vrmenus.limitstart', 'limitstart', 0, 'uint');
		$navbut = "";
		
		$q = "SELECT SQL_CALC_FOUND_ROWS * FROM `#__cleverdine_menus` 
		...
		for( $i = 0; $i < count($rows); $i++ ) {
			...
			$q = "SELECT `tag` FROM `#__cleverdine_lang_menus` WHERE `id_menu`=".$rows[$i]['id'].";";
			$dbo->setQuery($q);
			$dbo->execute();
			if( $dbo->getNumRows() > 0 ) {
		...
		$q = "SELECT MIN(`ordering`) AS `min`, MAX(`ordering`) AS `max` FROM `#__cleverdine_menus`;";
		$dbo->setQuery($q);
		$dbo->execute();
	...
	private function addToolBar() {
		...
		JToolbarHelper::title(JText::_('VRMAINTITLEVIEWMENUS'), 'restaurants');
		
		if (JFactory::getUser()->authorise('core.create', 'com_cleverdine')) {
			JToolbarHelper::addNew('newmenu', JText::_('VRNEW'));
			JToolbarHelper::divider();
		}
		if (JFactory::getUser()->authorise('core.edit', 'com_cleverdine')) {
			JToolbarHelper::editList('editmenu', JText::_('VREDIT'));
			JToolbarHelper::spacer();
			
			JToolbarHelper::editList('editmenuordering', JText::_('VREDITORDERING'));
			JToolbarHelper::spacer();    
		}
		if (JFactory::getUser()->authorise('core.delete', 'com_cleverdine')) {
			JToolbarHelper::deleteList( '', 'deleteMenus', JText::_('VRDELETE'));
		}
