...
<?php 
	if( count( $rows ) == 0 ) {
		?>
		<p><?php echo JText::_('VRNOLANGTKATTRIBUTE');?></p>
		<?php
	} else {
?>
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="<?php echo $vik->getAdminTableClass(); ?>">
		...
		for( $i = 0; $i < count($rows); $i++ ) {
			...
			<tr class="row<?php echo $kk; ?>">
				...
				<td><a href="index.php?option=com_cleverdine&task=editlangtkattribute&cid[]=<?php echo $row['id']; ?>"><?php echo $row['name']; ?></a></td>
	...
	<input type="hidden" name="task" value="langtkattributes"/>
	<input type="hidden" name="id_attribute" value="<?php echo $this->idAttribute; ?>"/>
