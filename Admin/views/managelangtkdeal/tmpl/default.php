	...
	<div class="span11">
		<?php echo $vik->openEmptyFieldset(); ?>
			...
			<?php echo $vik->openControl(JText::_('VRMANAGELANG2').":"); ?>
				<input type="text" name="deal_name" value="<?php echo (empty($this->struct['deal_lang_name']) ? $this->struct['deal_name'] : $this->struct['deal_lang_name']); ?>" size="48"/>
			<?php echo $vik->closeControl(); ?>
			
			<!-- DEAL DESCRIPTION - Textarea -->
			<?php echo $vik->openControl(JText::_('VRMANAGELANG3').":"); ?>
				<textarea name="deal_description" style="width:450px;height:150px;"><?php echo (empty($this->struct['deal_lang_description']) ? $this->struct['deal_description'] : $this->struct['deal_lang_description']); ?></textarea>
			<?php echo $vik->closeControl(); ?>
			
			<?php if( $this->type == "edit" ) { ?>
				<input type="hidden" name="id_lang_deal" value="<?php echo $this->struct['id_lang_deal']; ?>"/>
			<?php } ?>
